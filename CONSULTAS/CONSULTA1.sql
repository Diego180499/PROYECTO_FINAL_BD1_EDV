...
SELECT estudiante_nota.carne, estudiante_nota.nombre, estudiante_nota.promedio, ca.id, ins.fecha
FROM
	(SELECT con.carne, con.nombre, sum(con.zona_obtenida), sum(con.nota_examen_final), avg(zona_obtenida+nota_examen_final) as promedio 
    FROM
		(SELECT e.carne, e.nombre, an.zona_obtenida, an.nota_examen_final
...
WHERE p.promedio = (
	...
	WHERE p1.id = p.id
) AND p.fecha < '2026-01-01'; -- Fecha desde la que se verificaran promedios